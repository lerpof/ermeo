// Design token codegen for ermeo_ui.
//
...
String generatePrimitiveTokensDart(ParsedTokens tokens) {
  ...
  buffer.writeln('abstract final class ErPrimitiveColors {');
  buffer.writeln('  ErPrimitiveColors._();');
  ...
  buffer.writeln('abstract final class ErPrimitiveSpacing {');
  buffer.writeln('  ErPrimitiveSpacing._();');
  ...
  buffer.writeln('abstract final class ErPrimitiveRadius {');
  buffer.writeln('  ErPrimitiveRadius._();');
  ...
  buffer.writeln('class ErPrimitiveShadowDefinition {');
  buffer.writeln('  const ErPrimitiveShadowDefinition({');
  ...
  buffer.writeln('abstract final class ErPrimitiveShadows {');
  buffer.writeln('  ErPrimitiveShadows._();');
  final sortedShadowKeys = tokens.shadows.keys.toList()..sort();
  for (final key in sortedShadowKeys) {
    final shadow = tokens.shadows[key]!;
    buffer.writeln('  static const $key = ErPrimitiveShadowDefinition(');
    buffer.writeln(
  ...
  buffer.writeln('class ErPrimitiveTextStyleDefinition {');
  buffer.writeln('  const ErPrimitiveTextStyleDefinition({');
  ...
  buffer.writeln('abstract final class ErPrimitiveTypography {');
  buffer.writeln('  ErPrimitiveTypography._();');
  buffer.writeln(
  ...
  for (final key in sortedTypoKeys) {
    ...
    buffer.writeln('  static const $key = ErPrimitiveTextStyleDefinition(');
...
String generateSemanticTokenMapsDart(ParsedTokens tokens) {
  ...
  buffer.writeln('abstract final class ErSemanticColorMap {');
  ...
  buffer.writeln('abstract final class ErSemanticShadowMap {');
  ...
  buffer.writeln('abstract final class ErSemanticSpacingMap {');
  ...
  buffer.writeln('abstract final class ErSemanticRadiusMap {');
...
void _writeShadowMap(
  ...
  buffer.writeln('  static const $theme = <String, ErPrimitiveShadowDefinition>{');
  final sortedKeys = shadows.keys.toList()..sort();
  for (final key in sortedKeys) {
    final shadow = shadows[key]!;
    buffer.writeln("    '$key': ErPrimitiveShadowDefinition(");
    buffer.writeln(
      '      color: Color(0x${shadow.color.argb.toRadixString(16).padLeft(8, '0').toUpperCase()}),',
    );
    buffer.writeln('      alpha: ${shadow.alpha},');
